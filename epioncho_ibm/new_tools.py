    ...
    if prevalence or number or mean_worm_burden or intensity:
        if with_age_groups:
            ...
            add_state_to_run_data_helper(
                ...
                pct_non_compliant=pct_non_compliant,
            )
...
def add_state_to_run_data_helper(
    ...
    if pct_non_compliant:
        age_state, partial_key = return_age_state(
            state, output_age_range_map, "pnc", age_start, age_end
        )
        run_data[(*partial_key, "pnc")] = np.mean(age_state.people.has_been_treated)
