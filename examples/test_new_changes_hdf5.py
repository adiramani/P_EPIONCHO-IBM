...
def run_sim(i, iu_name, verbose=False, sample=True):
    ...
    new_file = h5py.File("/test_outputs/OutputVals_GHA0216121382.hdf5", "r")
    restored_grp = new_file["draw_" + str(i)]
    ...
    # Run
    run_data: Data = {}
    for state in sim.iter_run(end_time=2041, sampling_interval=1):
...
def wrapped_parameters(iu_name):
    ...
    num_iter = 1
    max_workers = os.cpu_count() - 1 if num_iter > os.cpu_count() - 1 else num_iter
    ...
    data = process_map(rumSim, range(num_iter), max_workers=max_workers)
    write_data_to_csv(data, "test_outputs/python_model_output/tmp2.csv")
