...
python_model_data_1 <- configure_python_data("../test_outputs/python_model_output/testing_GHA0216121382-sadasdassadaage_grouped_raw_data_variable_abr_366days_reset_zeros_pnc_updated_dd_daily_sampling.csv")
python_model_data_2 <- configure_python_data("../test_outputs/python_model_output/ihme-GHA0216121382-scenario_1ai-10_sims-mda_stop_2040-sampling_interval_1.0-raw_all_age_data.csv")
...
python_model_data_3 <- configure_python_data("../test_outputs/python_model_output/tmp2.csv")

# timestep = 1/2 a day
r_model_data_1 <- configure_r_data("../test_outputs/output_ghana/", isPNC=TRUE, timeStep = 0.5/366)
# timestep = 1 day
...
mfp_plot <- ggplot() +
    ...
    geom_line(aes(x=year_id, y=mean_mf_prev, color="Local Python Model"), alpha=0.5, data=python_model_data_1) +
    geom_line(aes(x=year_id, y=mean_mf_prev, color="Test Python Model"), alpha=0.5, data=python_model_data_2) +
    geom_line(aes(x=year_id, y=mean_mf_prev, color="Local Python Model Hdf5"), alpha=0.5, data=python_model_data_3) +
    scale_color_manual("Model", values=c(
                        "Original R Model"="black",
                        "Local Python Model"="blue",
                        "Test Python Model" = "red",
                        "Local Python Model Hdf5" = "purple"
                        ),
                        labels = function(x) str_wrap(x, width=16)) +
    scale_y_continuous("MF Prevalence (%)", limits=c(0, 1.1), breaks=seq(0, 1, 0.1)) +
...
ggsave("../test_outputs/comparison_plots.png", (mfp_plot / pnc_plot), 
...
pnc_plot <- ggplot() +
    ...
    scale_x_continuous("Time (years)", limits=c(2026, 2030), breaks=seq(1900, 2040, 10))
...
mfp_plot <- mfp_plot  + 
pnc_plot <- pnc_plot  + geom_line(aes(x=year_id, y=mean_pnc, color="Test Python Model"), alpha=0.5, data=python_model_data_2)
